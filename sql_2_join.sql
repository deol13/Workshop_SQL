...
#
SELECT countrycode, COUNT(*) AS nr FROM city WHERE countrycode IN ('MOZ','VNM') GROUP BY countrycode;
#
# 10: Get average population of cities in MOZ and VNM
#
SELECT countrycode, AVG(population) as num FROM city WHERE countrycode IN ('MOZ','VNM') GROUP BY countrycode;
#
# 11: Get the countrycodes with more than 200 cities
#
SELECT countrycode, COUNT(*) AS nr FROM city GROUP BY countrycode HAVING nr > 200; 
#
# 12: Get the countrycodes with more than 200 cities ordered by city count
#
SELECT countrycode, COUNT(*) AS nr FROM city GROUP BY countrycode HAVING nr > 200 ORDER BY nr; 
#
...
SELECT c1.name FROM city c1,city c2 WHERE c1.countrycode=c2.countrycode AND c2.population=122199 AND c1.population<>122199;
#
# My original solution, a bit too complex
SELECT city.name FROM city 
	JOIN country ON city.countrycode = country.code
...
#
# From Teacher
SELECT c2.name, cr2.region FROM city c1, country cr1, country cr2, city c2 WHERE c1.name = 'Yaren' AND c1.id = cr1.capital AND cr1.region = cr2.region AND cr2.capital = c2.id;
#
# My original solution, a bit too complex    
...
#
SELECT DISTINCT cl.language FROM country cr1 
JOIN countrylanguage cl ON cr1.code = cl.countrycode,
